...
class Modify(SubTransformer):

    name = "Modify"

    def __init__(self, low=0, high=1, number_type="float", action_samples=10, sample_method="random"):
        """
        Initialize a `Modify` object. This object modifies an object by a numerical amount depending on the hyper
        parameters.
        :param low: The lower bound (inclusive) to sample values from. It should be a non-negative number.
        :param high: The upper bound (exclusive) to sample values from. It should be a non-negative number.
    ...
    def transform(
        ...
        if transformation_value is None:
            if self.number_type == "int":
                new_value = np.random.randint(self.low, self.high)
            elif self.number_type == "float":
                new_value = np.random.uniform(self.low, self.high)
    ...
    def get_action_list(self):
        ...
        if self.sample_method == "random":
            ...
            if self.number_type == "int":
                increment_values = np.linspace(low, high, self.action_samples, endpoint=False, dtype=int)
            elif self.number_type == "float":
                increment_values = np.linspace(low, high, self.action_samples, endpoint=False)

        elif self.sample_method == "geomspace":
            if low == 0:
                low = 0.000001
            if self.number_type == "int":
                increment_values = np.geomspace(low, high, self.action_samples, endpoint=False, dtype=int)
            elif self.number_type == "float":
